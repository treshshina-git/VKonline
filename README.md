...
Бот для Telegram, который по команде **`/start`** (или кнопке **Start**) :
...
4) отправляет пользователю содержимое выбранной категории **обычным сообщением** (со ссылками на активные позиции).

> Дополнительно поддерживается alias команды **`/gogogogo`**.
